 ...
 * behind it, and a fast percentage count — which, at 100 %, triggers a
 * CSS clip-path wipe (right → left) that swaps "Loading 100 %" for
 * "Willkommen". A background expansion layer then scale()-animates to
 * swallow the screen (GPU-only, zero layout reflow), before the overlay
 * fades and reveals the hero underneath.
 *
...
export function Preloader() {
  ...
  // Kick off image preloads and the visual percentage tween.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setExitStarted(true), 1200)
    return () => window.clearTimeout(t)
  }, [wipeStarted])
