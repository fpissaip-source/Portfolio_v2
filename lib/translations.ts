...
export type Dictionary = {
  ...
  kiAntwort: {
    ...
    quelle: string
    /** Der Wortlaut der Antwort. Sichtbar ist das Bildschirmfoto; diese
     *  Abschrift steht daneben, damit auch etwas dasteht fuer jeden, der die
     *  Seite nicht ansieht, sondern ausliest. */
    absaetze: string[]
    bildAlt: string
    /** Worauf man achten soll. Keine Wiederholung der Antwort, sondern das,
...
export const EN: Dictionary = {
  ...
  lukas: {
    ...
    beats: [
      {
        kicker: 'Autonomy',
        title: 'He keeps working when nobody is watching.',
        body: [
          'Every thirty minutes lib/autonomy.ts asks whether there is anything to do: active goals, new replies, events. If nothing changed since the last run, no thinking happens at all.',
          'No fixed number of turns. If a task needs twenty commands, he takes twenty. The limit is a token and time budget per move, not a number in the code.',
        ],
      },
      {
        kicker: 'Security model',
        title: 'A language model is never an authorisation server.',
        body: [
          'Lukas decides what he wants to do. Whether it runs is decided by lib/policy.ts: after the model, before the tool that holds the real credentials.',
          'Four tiers. Reading and internal runs go through alone; anything with outside effect or money needs approval. A tool without a classification counts as needing approval.',
        ],
      },
      {
        kicker: 'Memory',
        title: 'Not a transcript. A graph.',
        body: [
          'Memories, goals, journal and episodes live in Postgres, retrieved through embeddings. On top of that a graph of nodes and edges, exportable as an Obsidian vault.',
          'Every tool call records its outcome. After three failures at the same thing, the prompt says what went wrong. Counted, not narrated.',
        ],
      },
      {
        kicker: 'Acting on the web',
        title: 'He sees what he is doing.',
        body: [
          'He operates pages in a permanently signed-in session: clicking, typing, uploading. What comes back is a screenshot.',
          'The text of a page does not tell you whether a cookie banner is sitting on top of the button. The picture does.',
        ],
      },
      {
        kicker: 'Verification',
        title: 'Every promise needs a counter-test.',
        body: [
          'Over thirty check scripts bundle the real module and replace only the database, the network and the model provider. They do not test that code exists, they test that it works.',
          'The standard: remove the defence and the test must fire. The DNS rebinding test passed at first and proved nothing. Now a real server runs against it.',
        ],
      },
    ],
...
export const DE: Dictionary = {
  ...
  lukas: {
    ...
    beats: [
      {
        kicker: 'Autonomie',
        title: 'Er arbeitet weiter, wenn niemand zusieht.',
        body: [
          'Alle dreißig Minuten fragt lib/autonomy.ts, ob es etwas zu tun gibt: aktive Ziele, neue Antworten, Ereignisse. Hat sich seit dem letzten Lauf nichts geändert, wird gar nicht erst gedacht.',
          'Keine feste Rundenzahl. Braucht eine Aufgabe zwanzig Befehle, macht er zwanzig. Die Grenze ist ein Token- und Zeitbudget pro Zug, keine Zahl im Code.',
        ],
      },
      {
        kicker: 'Sicherheitsmodell',
        title: 'Ein Sprachmodell ist nie ein Autorisierungsserver.',
        body: [
          'Lukas entscheidet, was er tun will. Ob es läuft, entscheidet lib/policy.ts: nach dem Modell, vor dem Werkzeug mit den echten Zugangsdaten.',
          'Vier Stufen. Lesen und interne Läufe gehen allein, Wirkung nach außen und alles mit Geld braucht Freigabe. Ein Werkzeug ohne Einstufung gilt automatisch als freigabepflichtig.',
        ],
      },
      {
        kicker: 'Gedächtnis',
        title: 'Kein Verlauf, sondern ein Graph.',
        body: [
          'Erinnerungen, Ziele, Tagebuch und Episoden liegen in Postgres, abgerufen wird über Einbettungen. Darüber ein Graph aus Knoten und Kanten, exportierbar als Obsidian-Vault.',
          'Jeder Werkzeugaufruf hinterlässt seinen Ausgang. Ab drei Fehlschlägen an derselben Sache steht im Prompt, woran es lag. Gezählt, nicht erzählt.',
        ],
      },
      {
        kicker: 'Handeln im Web',
        title: 'Er sieht, was er tut.',
        body: [
          'Er bedient Seiten in einer dauerhaft angemeldeten Sitzung: klicken, tippen, hochladen. Zurück kommt ein Bildschirmfoto.',
          'Der Text einer Seite verrät nicht, ob ein Cookie-Banner über dem Knopf liegt. Das Bild schon.',
        ],
      },
      {
        kicker: 'Prüfungen',
        title: 'Jede Zusage braucht eine Gegenprobe.',
        body: [
          'Über dreißig Prüfskripte bündeln das echte Modul und ersetzen nur Datenbank, Netz und Modellanbieter. Sie prüfen nicht, dass Code existiert, sondern dass er wirkt.',
          'Der Maßstab: die Abwehr wird entfernt, und der Test muss anschlagen. Der Test gegen DNS-Rebinding lief zuerst grün und bewies nichts. Jetzt läuft ein echter Server dagegen.',
        ],
      },
    ],
